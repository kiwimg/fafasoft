...
public class CustomtTypeDAOImpl extends BaseDAO {
    ...
    public void update(CustomType customType) {
        String sql = "UPDATE CUSTOMTYPE SET " +
                "INTEGRATION ='" + customType.getIntegration() + "'," +
                "DISCOUNT ='" + customType.getDiscount() + "'" +
                " WHERE CUSTOMNAME='" + customType.getTypename() + "'";
        try {
            super.update(sql,true);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void addCustomTypes(CustomType[] customTypes) {
        for (int i = 0; i < customTypes.length; i++) {
            ...
            if (customType != null) {
                if (customType.getTypename() != null && customType.getIntegration() > 0) {
    ...
    private static class CustomTypeRowMapper implements RowMapper {
